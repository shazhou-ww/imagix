...
export function registerEntityTools(registry: ToolRegistry) {
  ...
  registry.register({
    ...
    description:
      "List all characters in a world (excludes soft-deleted). Characters are agents with subjective agency (humans, animals, gods, etc.).",
    inputSchema: {
      type: "object",
  ...
  registry.register({
    ...
    description:
      "Create a character. Automatically creates a birth event at birthTime with $age=0, $name, $alive=true. Must reference a CHAR taxonomy node.",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
    ...
    handler: async (a) =>
      ...
      jsonResult(await charCtrl.create(a.worldId as string, a as any)),
  });

  registry.register({
    name: "get_character",
    description:
      "Get a character's static metadata. Use compute_entity_state for dynamic attributes.",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
        characterId: { type: "string", description: "Character ID ('chr...')" },
      },
      required: ["worldId", "characterId"],
    },
    handler: async (a) =>
      jsonResult(
        await charCtrl.getById(a.worldId as string, a.characterId as string),
      ),
  });
  ...
  registry.register({
    ...
    handler: async (a) => {
      await charCtrl.remove(a.worldId as string, a.characterId as string);
      return okResult("Character deleted.");
    },
  });
  ...
  registry.register({
    name: "end_character",
    description:
      "Mark a character as dead. Creates a death event ($alive=false). Death time must be after birth time. Optionally link a cause event.",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
  ...
  registry.register({
    ...
    description:
      "Revive a character — delete the death event and associated links.",
    inputSchema: {
      type: "object",
      properties: { worldId: wid, characterId: { type: "string" } },
      required: ["worldId", "characterId"],
    ...
    handler: async (a) =>
      jsonResult(
        await charCtrl.undoEnd(a.worldId as string, a.characterId as string),
      ),
  });
  ...
  registry.register({
    name: "list_things",
    description:
      "List all things in a world (excludes soft-deleted). Things are non-agent entities (artifacts, items, forces).",
    inputSchema: {
      type: "object",
      properties: { worldId: wid },
      required: ["worldId"],
  ...
  registry.register({
    name: "create_thing",
    description:
      "Create a thing. Automatically creates a creation event at creationTime with $age=0, $name, $alive=true. Must reference a THING taxonomy node.",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
        ...
        creationTime: {
          type: "number",
          description: "Creation time in epoch ms",
        },
      },
    ...
    handler: async (a) =>
      // biome-ignore lint/suspicious/noExplicitAny: MCP tool args
      jsonResult(await thingCtrl.create(a.worldId as string, a as any)),
  });
  ...
  registry.register({
    ...
    inputSchema: {
      ...
      properties: {
        worldId: wid,
        thingId: { type: "string", description: "Thing ID ('thg...')" },
      },
      required: ["worldId", "thingId"],
    },
    handler: async (a) =>
      jsonResult(
        await thingCtrl.getById(a.worldId as string, a.thingId as string),
      ),
  });
  ...
  registry.register({
    ...
    handler: async (a) => {
      await thingCtrl.remove(a.worldId as string, a.thingId as string);
      return okResult("Thing deleted.");
    },
  });
  ...
  registry.register({
    name: "end_thing",
    description:
      "Mark a thing as destroyed. Creates a destruction event ($alive=false).",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
  ...
  registry.register({
    ...
    handler: async (a) =>
      jsonResult(
        await thingCtrl.undoEnd(a.worldId as string, a.thingId as string),
      ),
  });
  ...
  registry.register({
    name: "list_places",
    description:
      "List all places in a world. Places are spatial containers with parent-child hierarchy. No lifecycle.",
    inputSchema: {
      type: "object",
      properties: { worldId: wid },
      required: ["worldId"],
  ...
  registry.register({
    ...
    inputSchema: {
      ...
      properties: {
        ...
        parentId: {
          type: ["string", "null"],
          description: "Parent place ID (null for top-level)",
        },
        description: { type: "string" },
      },
    ...
    handler: async (a) =>
      jsonResult(
        await placeCtrl.create(a.worldId as string, {
  ...
  registry.register({
    ...
    inputSchema: {
      ...
      properties: {
        worldId: wid,
        placeId: { type: "string", description: "Place ID ('plc...')" },
      },
      required: ["worldId", "placeId"],
    },
    handler: async (a) =>
      jsonResult(
        await placeCtrl.getById(a.worldId as string, a.placeId as string),
      ),
  });
  ...
  registry.register({
    ...
    handler: async (a) => {
      await placeCtrl.remove(a.worldId as string, a.placeId as string);
      return okResult("Place deleted.");
    },
  });
  ...
  registry.register({
    ...
    description:
      "List all relationships involving a specific entity (as source or target).",
    inputSchema: {
      type: "object",
      properties: {
    ...
    handler: async (a) =>
      jsonResult(await relCtrl.listByEntity(a.entityId as string)),
  });
}
