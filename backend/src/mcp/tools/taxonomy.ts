...
export function registerTaxonomyTools(registry: ToolRegistry) {
  ...
  registry.register({
    ...
    inputSchema: {
      ...
      properties: {
        worldId: wid,
        tree: { type: "string", enum: ["CHAR", "THING", "REL"], description: "Taxonomy tree type" },
      },
      required: ["worldId", "tree"],
    },
    handler: async (a) => jsonResult(await taxonomyCtrl.getTree(a.worldId as string, a.tree as TaxonomyTree)),
  });

  registry.register({
    name: "create_taxonomy_node",
    description: "Create a taxonomy node. Nodes form a hierarchy via parentId. Use timeFormula (JSONata) for time-dependent attribute computation.",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
        ...
        timeFormula: { type: ["string", "null"], description: "JSONata expression for time-dependent attrs" },
      },
  ...
  registry.register({
    ...
    description: "Update a taxonomy node (name, parentId, timeFormula). System preset nodes cannot be edited.",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
  ...
  registry.register({
    ...
    description: "Delete a taxonomy node. System preset nodes cannot be deleted.",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
    ...
    handler: async (a) => {
      await taxonomyCtrl.remove(a.worldId as string, a.tree as TaxonomyTree, a.nodeId as string);
  ...
  registry.register({
    ...
    description: "List attribute definitions for a world. These define what attributes can exist on entities, with types, defaults, enums, etc.",
    inputSchema: { type: "object", properties: { worldId: wid }, required: ["worldId"] },
    handler: async (a) => jsonResult(await attrDefCtrl.list(a.worldId as string)),
  });

  registry.register({
    name: "create_attribute_definition",
    description: "Create an attribute definition. Define custom attributes for entities (e.g. 'mood', 'power_level') with type and display info.",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
        ...
        enumValues: { type: "array", items: { type: "string" }, description: "Allowed values (string type only)" },
        scope: { type: "string", enum: ["entity", "relationship"], description: "Scope" },
      },
      required: ["worldId", "name", "label", "type", "scope"],
    },
    handler: async (a) => jsonResult(await attrDefCtrl.create(a.worldId as string, a as any)),
  });

  registry.register({
    name: "update_attribute_definition",
    description: "Update an attribute definition. System preset defs cannot be edited.",
    inputSchema: {
      type: "object",
      properties: {
        worldId: wid,
        attributeDefinitionId: { type: "string", description: "Attribute definition ID ('adf...')" },
        label: { type: "string" },
    ...
    handler: async (a) => {
      ...
      return jsonResult(await attrDefCtrl.update(worldId, attributeDefinitionId, body));
    },
  });

  registry.register({
    name: "delete_attribute_definition",
    description: "Delete an attribute definition. System preset defs cannot be deleted.",
    inputSchema: {
      type: "object",
    ...
    handler: async (a) => {
      await attrDefCtrl.remove(a.worldId as string, a.attributeDefinitionId as string);
